...
- [x] **WK2-01 — Data types, variables, and operators**
  Cover primitives, type coercion, `var` vs `let` vs `const`. Write small exercises — predict output before running.

- [x] **WK2-02 — Arrays and objects — methods and reference vs value**
  Cover primitives, type coercion, `var` vs `let` vs `const`. Write small exercises — predict output before running.

- [x] **WK2-03 — Functions — scope, closures, and arrow functions**
...
| Monday | WK2-01 | ✅ Done |
| Tuesday | WK2-02 | ✅ Done |
| Wednesday | WK2-03 | ✅ Done |
| Thursday | WK2-04 | ⬜ Pending |
| Friday | WK2-05 | ⬜ Pending |
| Saturday | WK2-06 | ⬜ Pending |
| — | WK2-07 to WK2-12 | ⬜ Pending |

*Note: "Make cards using flatList" from the original task dump wasn't included above — it references React Native's `FlatList`, which doesn't fit this vanilla JS/TS scope. Let me know if that's a separate task to track.*